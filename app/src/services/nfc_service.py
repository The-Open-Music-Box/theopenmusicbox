...
class NFCService:
    ...
    async def start_listening(self, playlist_id: str, sid: Optional[str] = None) -> None:
        """
        ...
        if self._nfc_handler and not self._nfc_handler.is_running():
            try:
                await self._nfc_handler.start_nfc_reader()
                logger.log(LogLevel.INFO, f"NFC hardware reader started for playlist {playlist_id}")
            except Exception as e:
        ...
        logger.log(LogLevel.INFO, f"Started listening for NFC tag association with playlist {playlist_id}")
    ...
    async def handle_tag_detected(self, tag_id: str, tag_data: Optional[Dict[str, Any]] = None) -> bool:
        """
        ...
        self._last_tag_info = {'tag_id': tag_id, 'timestamp': asyncio.get_event_loop().time()}
        
        # Log current mode for debugging
        logger.log(LogLevel.INFO, f"Handling tag in {'association' if self._association_mode else 'playback'} mode")
        
        # If not in association mode, handle as playback tag
        if not self._association_mode:
            # Only propagate the event to the controller if we're not in association mode
            return await self._handle_playback_tag(tag_id, tag_data)
        
        # From here on, we're in association mode
        # Emit an event to inform the client that a tag has been detected while in association mode
        if self.socketio and self._sid:
            await self.socketio.emit('nfc_tag_detected', {
            ...
            logger.log(LogLevel.INFO, f"Emitted nfc_tag_detected event to client {self._sid}")
        
        # If not waiting for a tag, ignore
        if not self.waiting_for_tag:
            logger.log(LogLevel.WARNING, "Tag detected but not waiting for tag association")
            return False
        ...
        if associated_playlist and associated_playlist['id'] != self.current_playlist_id:
            status_update = {
                'type': 'nfc_status',
                ...
                'associated_playlist_title': associated_playlist.get('title', 'Unknown')
            }
            
            if self.socketio and self._sid:
                await self.socketio.emit('nfc_status', status_update, room=self._sid)
                logger.log(LogLevel.INFO, f"Emitted 'already_associated' status to client {self._sid}")
                    
            logger.log(LogLevel.WARNING, f"Tag {tag_id} already associated with playlist {associated_playlist['id']}")
            return False
            
        # If tag is free or we're overriding, associate it with the current playlist
        if self.current_playlist_id and (self._allow_override or not associated_playlist):
            # Find current playlist in the mapping
            current_playlist = next((p for p in self._playlists if p['id'] == self.current_playlist_id), None)
            if current_playlist:
                ...
                if associated_playlist and self._allow_override:
                    ...
                    logger.log(LogLevel.INFO, f"Removed tag {tag_id} from playlist {old_playlist_title}")
                ...
                try:
                    ...
                    if playlist_service:
                        result = playlist_service.associate_nfc_tag(self.current_playlist_id, tag_id)
                        if result:
                            logger.log(LogLevel.INFO, f"Tag {tag_id} successfully persisted to database for playlist {self.current_playlist_id}")
                        else:
                            logger.log(LogLevel.ERROR, f"Failed to persist tag {tag_id} association in database")
                    else:
                        logger.log(LogLevel.ERROR, "Could not access playlist service to persist NFC tag association")
                except Exception as e:
                ...
                if self.socketio and self._sid:
                    await self.socketio.emit('nfc_status', status_update, room=self._sid)
                    logger.log(LogLevel.INFO, f"Emitted 'success' status to client {self._sid}")
    ...
    async def _handle_playback_tag(self, tag_id: str, tag_data: Optional[Dict[str, Any]] = None) -> bool:
        """
        ...
        if self._association_mode:
            logger.log(LogLevel.WARNING, "Association mode detected in _handle_playback_tag - this should not happen")
            return False
            
        # Emit the event to the playback subject
        try:
            logger.log(LogLevel.INFO, f"Playback mode: Emitting tag {tag_id} to playback subject")
                ...
                while self.waiting_for_tag and self._association_mode:
                    if self.socketio and self._sid:
                        ...
                        if self._last_tag_info:
                            ...
                            status_update['last_tag_timestamp'] = self._last_tag_info['timestamp']
                            
                        await self.socketio.emit('nfc_scanning', status_update, room=self._sid)
    ...
    async def handle_tag_association(self, tag_id: str, tag_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not self._association_mode or not self.current_playlist_id:
            logger.log(LogLevel.WARNING, "Tag association attempted but not in association mode")
        ...
        if existing_playlist and not self._allow_override:
            logger.log(LogLevel.WARNING, f"Tag {tag_id} already associated with playlist {existing_playlist.get('id')}")
            
            if self.socketio and self._sid:
                await self.socketio.emit('nfc_association_result', {
                    'status': 'error',
