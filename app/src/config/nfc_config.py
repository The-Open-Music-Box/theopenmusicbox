...
class NFCConfig:
    ...
    # Playback control
    manual_action_priority_window: float = (
        1.0  # Window for manual actions to override NFC
    )
    ...
    mock_detection_interval: float = (
        5.0  # Interval for simulated tag detection in seconds
    )
    ...
    def validate(self) -> None:
        ...
        if self.tag_cooldown < 0:
            raise ValueError(
                f"tag_cooldown must be non-negative, got {self.tag_cooldown}"
            )

        if self.pause_threshold < 0:
            raise ValueError(
                f"pause_threshold must be non-negative, got {self.pause_threshold}"
            )
