...
def get_nfc_service(request: Request) -> NFCService:
    ...
    logger.log(
        LogLevel.DEBUG,
        f"get_nfc_service: Retrieving NFC service from request.app.container",
    )
    container = getattr(request.app, "container", None)
    if not container:
        logger.log(
            LogLevel.WARNING, "get_nfc_service: Container not available in request.app"
        )
        return None
    ...
    if not nfc_service:
        logger.log(
            LogLevel.WARNING, "get_nfc_service: NFC service not available in container"
        )
    else:
        logger.log(
            LogLevel.DEBUG, f"get_nfc_service: NFC service retrieved successfully"
        )
    ...
    def _register_routes(self):
        ...
        async def observe_nfc(
            ...
            try:
                ...
                if (
                    not nfc_service
                    or not hasattr(nfc_service, "_nfc_handler")
                    or not nfc_service._nfc_handler
                ):
                    logger.log(LogLevel.WARNING, "NFC reader not available")
                    return JSONResponse(
                        status_code=503, content={"error": "NFC reader not available"}
                    )
                ...
                if result.get("status") == "ok":
                    ...
                    logger.log(
                        LogLevel.ERROR,
                        f"Failed to start NFC observation: {result.get('message', 'Unknown error')}",
                    )
                    return JSONResponse(
                        status_code=500,
                        content={"error": result.get("message", "Unknown error")},
                    )
            except Exception as e:
                logger.log(LogLevel.ERROR, f"Failed to start NFC observation: {e}")
                return JSONResponse(
                    status_code=500,
                    content={"error": f"Failed to start NFC observation: {str(e)}"},
                )
        ...
        async def link_nfc_tag(
            ...
            try:
                ...
                if not playlist_id or not tag_id:
                    return JSONResponse(
                        {"error": "playlist_id and tag_id are required"},
                        status_code=400,
                    )
                ...
                if existing_playlist:
                    if existing_playlist["id"] == playlist_id:
                        await self.socketio.emit(
                            "nfc_link_success",
                            {"playlist_id": playlist_id, "tag_id": tag_id},
                        )
                        return JSONResponse(
                            {
                ...
                if success:
                    ...
                    await self.socketio.emit(
                        "nfc_link_error", {"error": "Failed to associate tag"}
                    )
                    return JSONResponse(
                        {"error": "Failed to associate tag"}, status_code=500
                    )
            except Exception as e:
        ...
        async def cancel_nfc_observation(
            ...
            try:
                ...
                logger.log(
                    LogLevel.ERROR, f"Error cancelling NFC observation: {str(e)}"
                )
                await self.socketio.emit("nfc_link_error", {"error": str(e)})
                return JSONResponse({"error": str(e)}, status_code=500)
        ...
        async def stop_nfc_listening(
            ...
            try:
                ...
                await nfc_service.stop_listening()
                return JSONResponse(
                    {"status": "success", "message": "NFC listening stopped"},
                    status_code=200,
                )
            except Exception as e:
        ...
        async def simulate_tag_detection(
            ...
            try:
                ...
                if not data or "tag_id" not in data:
                    return JSONResponse(
                        {"status": "error", "message": "tag_id missing"},
                        status_code=400,
                    )
                tag_id = data["tag_id"]
