...
class APIRoutes:
    ...
    def __init__(self, app: FastAPI, socketio, container: ContainerAsync):
        ...
        if self.container.nfc:
            logger.log(LogLevel.INFO, f"APIRoutes: NFC service available, initializing NFC routes")
            self.nfc_routes = NFCRoutes(app, socketio, self.container.nfc)
            self.websocket_handlers = WebSocketHandlersAsync(socketio, app, self.container.nfc)
        else:
            logger.log(LogLevel.WARNING, "APIRoutes: NFC service not available from container. NFC-dependent routes will not be initialized.")
    ...
    def init_routes(self):
        ...
        try:
            ...
            logger.log(LogLevel.ERROR, f"Failed to initialize routes: {str(e)}", exc_info=True)
            raise
