...
from fastapi import APIRouter, Depends, HTTPException, Body, FastAPI, Request
...
class SystemRoutes:
    ...
    def _register_routes(self):
        ...
        async def set_system_volume(
            volume: int = Body(..., embed=True, ge=0, le=100),
            ...
            logger.log(LogLevel.INFO, f"API: Received request to set volume to {volume}")
            success = audio.set_volume(volume) # audio.set_volume expects 0-100
            if not success:
                logger.log(LogLevel.ERROR, f"API: Failed to set volume to {volume} via audio module")
                raise HTTPException(status_code=500, detail="Failed to set volume")

            current_volume = getattr(audio, '_volume', "unknown") # Get the updated volume
            logger.log(LogLevel.INFO, f"API: Volume successfully set to {current_volume}")
        ...
        async def get_system_volume(audio=Depends(get_audio)):
            ...
            current_volume = getattr(audio, '_volume', "unknown") # audio._volume should be 0-100
        ...
        async def api_health_check(request: Request):
            """Consolidated health check endpoint for the entire application"""
