...
class AudioProgressTracker:
    ...
    def __init__(
        ...
        self._is_busy_callback = (
            is_busy_callback  # Function to check if playback is active
        )
        ...
        while not self._stop_progress:
            ...
            if self._state_manager.is_playing:
                ...
                current_playing_state = (
                    self._is_busy_callback() if self._is_busy_callback else False
                )
    ...
    def _update_progress(self) -> None:
        ...
        try:
            ...
            # If internal state indicates 'playing' but hardware says it's stopped,
            # this is an error
            if self._state_manager.is_playing and not busy:
                logger.log(
                    LogLevel.WARNING,
                    "Audio state mismatch: state indicates playing but hardware says it's not",
                )
                return

            # Get current position and track duration
            elapsed = self._state_manager.current_position
            total = self._state_manager.get_track_duration(
                self._state_manager.current_track.path
            )
            ...
            playlist_info = (
                self._state_manager.playlist.to_dict()
                if self._state_manager.playlist
                and hasattr(self._state_manager.playlist, "to_dict")
                else None
            )
