...
class ControlesMock:
    ...
    Simulates hardware behavior for buttons and rotary encoders.
    Always reports as available and provides simulated responses.
    """
    ...
    def setup_input(self, pin: int, pull_up: bool = True) -> None:
        ...
        with self._lock:
            ...
            logger.log(LogLevel.DEBUG, f"Set up mock input on pin {pin} (pull_up={pull_up})")
    ...
    def setup_button(self, pin: int, callback: Callable[[bool], None], pull_up: bool = True) -> None:
        ...
        with self._lock:
            ...
            logger.log(LogLevel.DEBUG, f"Set up mock button on pin {pin} (pull_up={pull_up})")
    ...
    def setup_rotary_encoder(self, 
        ...
        with self._lock:
            ...
            self._rotary_callbacks[clk_pin] = {
                'dt_pin': dt_pin,
                'callback': callback
            }
            
            logger.log(LogLevel.DEBUG, f"Set up mock rotary encoder on CLK={clk_pin}, DT={dt_pin}")
    ...
    def simulate_rotary_turn(self, clk_pin: int, clockwise: bool = True) -> None:
        ...
        if clk_pin not in self._rotary_callbacks:
            logger.log(LogLevel.WARNING, f"No rotary encoder callback registered for CLK pin {clk_pin}")
            return
        ...
        logger.log(LogLevel.DEBUG, f"Simulating rotary encoder turn ({direction}) on CLK pin {clk_pin}")
        callback(clockwise)
