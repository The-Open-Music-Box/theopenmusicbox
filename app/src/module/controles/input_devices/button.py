...
Provides a Button class that wraps the hardware-specific button implementation
and emits control events when pressed or released.
"""
...
class Button:
    ...
    Wraps a physical button connected to a GPIO pin and emits control events
    ...
    def __init__(self, 
        ...
        if hardware.is_available():
            ...
            logger.log(LogLevel.WARNING, f"Hardware not available. Button '{name}' will not function.")
    ...
    def _on_button_state_change(self, pressed: bool) -> None:
        ...
        if pressed:
            ...
            logger.log(LogLevel.DEBUG, f"Button '{self.name}' pressed, emitting {self.event_type.name}")
