...
Provides a RotaryEncoder class that wraps the hardware-specific rotary
encoder implementation and emits control events when rotated clockwise
...
class RotaryEncoder:
    ...
    Wraps a physical KY-040 rotary encoder connected to GPIO pins and
    emits control events when rotated clockwise or counter-clockwise.
    """
    ...
    def __init__(
        ...
        if hardware.is_available():
            ...
            hardware.setup_rotary_encoder(clk_pin, dt_pin, self._on_rotation, pull_up)
            ...
            logger.log(
                LogLevel.WARNING,
                f"Hardware not available. Rotary encoder '{name}' will not function.",
            )
    ...
    def _on_switch_change(self, pressed: bool) -> None:
        ...
        if pressed and self.sw_event_type is not None:
            event = ControlesEvent(
                ...
                metadata={"sw_pin": self.sw_pin, "pressed": pressed},
            )
