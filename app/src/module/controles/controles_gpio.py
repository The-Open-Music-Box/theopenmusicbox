...
This implementation handles direct communication with GPIO pins
...
class ControlesGPIO:
    ...
    Uses gpiozero with lgpio backend to interact with physical GPIO pins.
    Manages pin setup, event detection, and cleanup for buttons and rotary encoders.
    """
    ...
    def __init__(self):
        ...
        if not GPIO_AVAILABLE:
            logger.log(LogLevel.WARNING, "GPIO libraries not available. Hardware controls disabled.")
    ...
    def setup_input(self, pin: int, pull_up: bool = True) -> None:
        ...
        try:
            ...
            logger.log(LogLevel.ERROR, f"Failed to set up GPIO input on pin {pin}: {str(e)}")
    ...
    def setup_button(self, pin: int, callback: Callable[[bool], None], pull_up: bool = True) -> None:
        ...
        try:
            ...
            logger.log(LogLevel.ERROR, f"Failed to set up button on GPIO pin {pin}: {str(e)}")
    ...
    def setup_rotary_encoder(self, 
        ...
        try:
            ...
            clk_device.when_activated = lambda: self._handle_rotary_state_change(clk_pin)
            clk_device.when_deactivated = lambda: self._handle_rotary_state_change(clk_pin)
            
            logger.log(LogLevel.DEBUG, f"Set up rotary encoder on GPIO pins CLK={clk_pin}, DT={dt_pin}")
        except Exception as e:
            logger.log(LogLevel.ERROR, f"Failed to set up rotary encoder on GPIO pins CLK={clk_pin}, DT={dt_pin}: {str(e)}")
