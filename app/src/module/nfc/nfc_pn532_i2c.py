...
from app.src.helpers.exceptions import ErrorSeverity
...
class PN532I2CNFC(NFCHardware):
    ...
    This class handles the low-level communication with the PN532 NFC
    reader via I2C, while delegating tag detection logic to the
    TagDetectionManager.
    """
    ...
    async def _initialize_hardware(self) -> None:
        ...
        try:
            async with self._lock:
                ...
                logger.log(
                    LogLevel.INFO,
                    f"Found PN532 with firmware version: {ic} {ver}.{rev} - {support}",
                )
        ...
        while not self._stop_event.is_set():
            try:
                if error_count >= self._config.max_errors:
                    logger.log(
                        LogLevel.WARNING, "Max errors reached, reinitializing hardware"
                    )
                    await self._initialize_hardware()
                    error_count = 0
    ...
    async def stop_nfc_reader(self) -> None:
        ...
        Signals the reader loop to stop and cancels the task if
        necessary.
        """
        # Signal to stop
