...
class _MockPWM:
    """
    Private mock class for simulating PWM for tests and mock environments.
    """
    ...
    def start(self, duty_cycle: float) -> None:
        ...
        self.running = True
        print(f"[_MockPWM] Started with duty cycle {duty_cycle}%")

    def change_duty_cycle(self, duty_cycle: float) -> None:
        self.duty_cycle = duty_cycle
        print(f"[_MockPWM] Changed duty cycle to {duty_cycle}%")

    def stop(self) -> None:
        self.running = False
        print("[_MockPWM] Stopped")

class MockGPIO(GPIOHardware):
    """
