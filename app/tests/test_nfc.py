...
def test_initiate_nfc_association_missing_playlist():
    ...
    with TestClient(app) as client:
        response = client.post("/api/nfc/associate/initiate", json={})
...
def test_initiate_nfc_association_success():
    ...
    with TestClient(app) as client:
        response = client.post("/api/nfc/associate/initiate", json={"playlist_id": playlist_id})
...
def test_simulate_tag_detection_missing_tag():
    with TestClient(app) as client:
        response = client.post("/api/nfc/simulate_tag", json={})
    assert response.status_code == 400
...
def test_disassociate_nfc_tag_missing_fields():
    with TestClient(app) as client:
        response = client.post("/api/nfc/disassociate", json={})
    assert response.status_code == 400
