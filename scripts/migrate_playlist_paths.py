"""Migration script to clean up playlist 'path' fields by removing any leading
'uploads/' from the path. This script updates the playlist storage (JSON or DB)
in-place.
...
def migrate_json():
    if not PLAYLIST_JSON_PATH.exists():
        print(
            f"No playlist JSON file found at {PLAYLIST_JSON_PATH}, skipping JSON migration."
        )
        return
...
def migrate_sqlite():
    if not PLAYLIST_DB_PATH.exists():
        print(
            f"No playlist DB file found at {PLAYLIST_DB_PATH}, skipping SQLite migration."
        )
        return
    ...
    try:
        ...
        for pid, orig in rows:
            if orig and orig.startswith("uploads/"):
                ...
                cur.execute(
                    "UPDATE playlists SET path = ? WHERE id = ?", (new_path, pid)
                )
