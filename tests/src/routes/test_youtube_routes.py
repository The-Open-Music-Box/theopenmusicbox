...
def test_download_youtube_success(mock_flask_app, mock_socketio):
    ...
    with patch(
        "app.src.routes.youtube_routes.YouTubeService"
    ) as mock_service_class, mock_flask_app.app_context():
...
def test_download_youtube_error(mock_flask_app, mock_socketio):
    ...
    with patch(
        ...
        mock_service_instance = MagicMock()
        mock_service_instance.process_download.side_effect = Exception(
            "Download failed"
        )
        mock_service_class.return_value = mock_service_instance
...
@pytest.mark.api
@pytest.mark.timeout(5)  # Add timeout to prevent test hanging
def test_download_youtube_with_real_service_mock(
    mock_flask_app, mock_socketio, tmp_path
):
    ...
    to test the integration between the route and service without making
    ...
    with patch(
        "app.src.services.youtube.service.PlaylistService", MockPlaylistService
    ), patch(
        "app.src.services.youtube.service.YouTubeDownloader", MockYouTubeDownloader
