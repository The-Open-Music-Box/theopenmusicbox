...
These tests verify the functionality of the WebSocket handlers that
manage real-time communication between the backend and frontend.
"""
...
try:
    ...
    from app.src.services.notification_service import PlaybackEvent, PlaybackSubject
except ImportError:
...
def ws_handlers(mock_sio, mock_app, mock_nfc_service):
    ...
    if "pause_track" not in mock_sio.event_handlers:
        ...
        async def handle_pause_track(sid):
            try:
                ...
                await mock_sio.emit(
                    "track_paused",
                    {"status": "error", "message": f"No audio player found: {str(e)}"},
                    room=sid,
                )
    ...
    if (
        "override_nfc_tag" not in mock_sio.event_handlers
        and "handle_override_tag" in mock_sio.event_handlers
    ):
        mock_sio.event_handlers["override_nfc_tag"] = mock_sio.event_handlers[
            "handle_override_tag"
        ]
...
async def test_websocket_connect_handler(ws_handlers):
    ...
    with patch.object(PlaybackSubject, "get_instance") as mock_get_instance:
        ...
        progress_event = PlaybackEvent(
            "progress", {"current_time": 10.5, "duration": 180.0}
        )
        ...
        mock_sio.emit.assert_any_call("playback_status", status_event.data, room=sid)

        # Verify that the last progress was emitted
        mock_sio.emit.assert_any_call("track_progress", progress_event.data, room=sid)
...
async def test_set_playlist_handler(ws_handlers):
    ...
    async def test_set_playlist_handler(sid, data):
        ...
        try:
            ...
            await mock_sio.emit(
                "playlist_set", {"status": "ok", "playlist_id": data["id"]}, room=sid
            )
        except Exception as e:
            await mock_sio.emit(
    ...
    try:
        ...
        mock_sio.emit.assert_called_with(
            "playlist_set", {"status": "ok", "playlist_id": "test-playlist"}, room=sid
        )
    finally:
...
async def test_set_playlist_handler_no_player(ws_handlers):
    """Test the set_playlist event handler when no audio player is
    available."""
    ...
    mock_app_no_player.container.audio.get_player = AsyncMock(
...
async def test_play_track_handler(ws_handlers):
    ...
    async def test_play_handler(sid, data):
        ...
        try:
            ...
            await mock_sio.emit(
                "track_played", {"status": "ok", "track_number": track_number}, room=sid
            )
        except Exception as e:
            await mock_sio.emit(
    ...
    try:
        # Create test data
        sid = "test-sid"
        track_data = {"track_number": 1, "playlist_id": "test-playlist"}
        ...
        # Verify that the player's play_track method was called with the correct
        # track number
        audio_player.play_track.assert_called_once_with(1)

        # Check that a confirmation was emitted
        mock_sio.emit.assert_called_with(
            "track_played", {"status": "ok", "track_number": 1}, room=sid
        )
    finally:
        # Restore the original handler
...
async def test_stop_track_handler(ws_handlers):
    ...
    async def test_stop_handler(sid):
        ...
        try:
            ...
            await mock_sio.emit(
                "track_stopped", {"status": "error", "message": str(ex)}, room=sid
            )
...
async def test_pause_track_handler(ws_handlers):
    ...
    async def test_pause_handler(sid):
        ...
        try:
            ...
            await mock_sio.emit(
                "track_paused", {"status": "error", "message": str(ex)}, room=sid
            )
...
async def test_start_nfc_link_handler(ws_handlers):
    ...
    # Get handlers from the ws_handlers fixture
    mock_sio = ws_handlers.sio
    # Use the direct reference to the mock_nfc_service
    mock_nfc_service = ws_handlers.nfc_service
    ...
    # Create test data
    sid = "test-sid"
    data = {"playlist_id": "test-playlist"}
...
async def test_override_nfc_tag_handler(ws_handlers):
    ...
    # Verify that the NFC service set_override_mode method was called with the
    # correct value
