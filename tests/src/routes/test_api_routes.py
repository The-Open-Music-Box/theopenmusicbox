...
from app.src.routes.api_routes import APIRoutes, init_routes
...
class TestAPIRoutes:
    ...
    def test_api_routes_initialization(
        ...
        MockNFCRoutes.assert_called_once_with(mock_app, mock_socketio, mock_nfc_instance)
        MockYouTubeRoutes.assert_called_once_with(mock_app, mock_socketio)
        MockWebSocketHandlersAsync.assert_called_once_with(mock_socketio, mock_app, mock_nfc_instance)
    ...
    def test_api_routes_init_routes_success(
        self, 
        mock_logger, 
        mock_app, 
        mock_socketio
    ...
    @patch('app.src.routes.api_routes.logger')
    def test_api_routes_init_routes_failure(
        self, 
        mock_logger, 
        mock_app, 
        mock_socketio
        ...
        api_routes.youtube_routes = MagicMock(spec=YouTubeRoutes)
        api_routes.youtube_routes.register.side_effect = Exception("Test registration error")
        api_routes.websocket_handlers = MagicMock(spec=WebSocketHandlersAsync)
        api_routes.playlist_routes = MagicMock(spec=PlaylistRoutes)
        ...
        expected_logs = [
            call(LogLevel.INFO, "Initializing routes"),
            call(LogLevel.ERROR, "Failed to initialize routes: Test registration error")
        ]
        mock_logger.log.assert_has_calls(expected_logs, any_order=False)
