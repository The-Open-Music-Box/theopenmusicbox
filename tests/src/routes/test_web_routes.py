...
These tests verify the functionality of the web routes that serve static files
and provide health check information.
"""
import pytest
import os
...
def test_serve_file_exception(mock_flask_app):
    ...
    with patch.object(mock_flask_app, 'send_static_file', 
...
def test_health_check_exception(mock_flask_app):
    ...
    with mock_flask_app.test_client() as client:
        ...
        assert 'Container error' in data['message']  # Use 'in' for more flexible matching
