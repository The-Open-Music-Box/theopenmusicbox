...
These tests verify the functionality of the PlaybackSubject and
notification mechanisms that handle real-time updates between the
...
from app.src.services.notification_service import DownloadNotifier, PlaybackSubject


# Factory function for test doubles instead of class to avoid pytest collection warnings
def create_mock_socketio():
    """Create a test double for SocketIO that properly handles async
    operations."""
    mock = MagicMock()
    mock.emitted_events = []
...
def test_playback_subject_last_events(reset_playback_subject):
    ...
    with patch.object(subject, "_emit_socketio_event"):
        # Notify status
        subject.notify_playback_status(
            "playing", {"id": "test-playlist"}, {"title": "Test Track"}
        )
...
def test_playback_subject_notify_status_emits_socketio(reset_playback_subject):
    ...
    with patch.object(subject, "_emit_socketio_event") as mock_emit_event:
        subject.notify_playback_status(
            status_data["status"], status_data["playlist"], status_data["current_track"]
        )
...
def test_playback_subject_emit_socketio_with_loop(reset_playback_subject):
    """Test that _emit_socketio_event calls Socket.IO emit with an existing
    event loop."""
...
def test_playback_subject_throttle_progress(reset_playback_subject):
    """Test that progress events are throttled by directly testing the
    throttling logic."""
    ...
    event_data = {"current_time": 30.5, "duration": 180.0}
    ...
    with patch("time.time") as mock_time:
        ...
        is_throttled = (
            time_diff < 0.25
        )  # This is the actual throttle logic we're testing
...
def test_download_notifier():
    ...
    with patch.object(mock_socketio, "emit"):
        ...
        mock_socketio.emit.assert_called_with(
            "download_progress", {"download_id": download_id, "status": "started"}
        )
