...
@pytest.mark.api
@pytest.mark.timeout(2)
def test_playback_subject_streams(reset_playback_subject):
    """Test the status and progress streams of PlaybackSubject."""
    # Get the subject
    subject = PlaybackSubject.get_instance()
    
    # Verify the streams exist
    assert subject.status_stream is not None
    assert subject.progress_stream is not None
...
@pytest.mark.api
@pytest.mark.timeout(2)
def test_playback_subject_notify_status_rxpy(reset_playback_subject):
    """Test that notify_playback_status emits to RxPy subjects."""
    # Get the subject
    subject = PlaybackSubject.get_instance()
    ...
    subscription = subject.status_stream.subscribe(on_next)
    
    # Mock _emit_socketio_event to avoid real Socket.IO emission
    with patch.object(subject, '_emit_socketio_event'):
        # Notify status
        subject.notify_playback_status('playing', {'id': 'test-playlist'}, {'title': 'Test Track'})
        
        # Verify the event was received by the observer
        assert len(received_events) == 1
        assert received_events[0].event_type == 'status'
        assert received_events[0].data['status'] == 'playing'
...
@pytest.mark.api
@pytest.mark.timeout(2)
def test_playback_subject_notify_progress_rxpy(reset_playback_subject):
    """Test that notify_track_progress emits to RxPy subjects."""
    # Get the subject
    subject = PlaybackSubject.get_instance()
    ...
    subscription = subject.progress_stream.subscribe(on_next)
    ...
    with patch.object(subject, '_emit_socketio_event'):
        ...
        # Verify the event was received by the observer
        assert len(received_events) == 1
        assert received_events[0].event_type == 'progress'
        assert received_events[0].data['current_time'] == 30.5
        assert received_events[0].data['duration'] == 180.0
