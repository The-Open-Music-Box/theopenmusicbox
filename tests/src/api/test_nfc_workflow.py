...
async def test_observe_nfc_missing_playlist():
    ...
    # Status code 503 is expected because we're directly testing the
    # NFCHandler which might not be available
...
async def test_observe_nfc_playlist_not_found():
    ...
    # Service unavailable (503) or playlist not found (404)
...
async def test_cancel_nfc_observation():
    ...
    # With the new asynchronous version, the service state might not be
...
async def test_full_nfc_workflow(monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        res = client.post(
            "/api/nfc/link", json={"playlist_id": playlist_id, "tag_id": tag_id}
        )
        ...
        res = client.post(
            "/api/nfc/link", json={"playlist_id": other_playlist_id, "tag_id": tag_id}
        )
        ...
        res = client.post(
            "/api/nfc/link",
            json={"playlist_id": other_playlist_id, "tag_id": tag_id, "override": True},
        )
        # Accept 200 (success) or 503 (unavailable)
        assert res.status_code in [200, 503]
        if res.status_code == 200:
