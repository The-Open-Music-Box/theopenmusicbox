...
These tests verify the functionality of the Pygame audio backend
implementation.
"""
...
def mock_pygame_objects():
    """Create a comprehensive mock for the entire pygame module and its
    submodules."""
    ...
    # Use the real constant if available, or mock to an int
    pg_mock.USEREVENT = pygame.USEREVENT
...
def pygame_audio_backend(mock_pygame_objects, mock_mutagen_mp3):
    ...
    with patch(
        "app.src.module.audio_player.pygame_audio_backend.pygame", mock_pygame_objects
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_initialize(pygame_audio_backend, mock_pygame_objects):
    """Test initialization of PygameAudioBackend."""
    # Patch os.name and os.uname for consistent testing of initialization paths
    with patch(
        "app.src.module.audio_player.pygame_audio_backend.os.name", "posix"
    ), patch(
        "app.src.module.audio_player.pygame_audio_backend.os.uname",
        MagicMock(return_value=MagicMock(machine="x86_64", sysname="Darwin")),
    ):
    ...
    assert (
        pygame_audio_backend._initialized is True
    ), "backend should be marked as initialized"

    mock_pygame_objects.mixer.init.assert_called_once_with()  # Default init for macOS
    # Check that global pygame.init() was called
    mock_pygame_objects.init.assert_called_once()
    mock_pygame_objects.mixer.music.set_endevent.assert_called_once_with(
        mock_pygame_objects.USEREVENT + 1
    )
    mock_pygame_objects.mixer.music.set_volume.assert_called_once_with(
        0.5
    )  # Default volume 50%
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_shutdown(pygame_audio_backend, mock_pygame_objects):
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_load(pygame_audio_backend, mock_pygame_objects):
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_play(pygame_audio_backend, mock_pygame_objects):
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_pause_resume(pygame_audio_backend, mock_pygame_objects):
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_stop(pygame_audio_backend, mock_pygame_objects):
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_set_volume(pygame_audio_backend, mock_pygame_objects):
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_get_volume(pygame_audio_backend, mock_pygame_objects):
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_is_playing(pygame_audio_backend, mock_pygame_objects):
...
@pytest.mark.audio
# No mock_pygame_objects needed if only setting attribute
def test_pygame_audio_backend_register_end_event_callback(pygame_audio_backend):
...
@pytest.mark.audio
# mock_mutagen_mp3 is still fine
def test_pygame_audio_backend_get_duration(pygame_audio_backend, mock_mutagen_mp3):
...
@pytest.mark.audio
# Use mock_pygame_objects
def test_pygame_audio_backend_process_events(pygame_audio_backend, mock_pygame_objects):
