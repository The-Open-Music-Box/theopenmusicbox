...
These tests verify the functionality of the WM8960 hardware audio player
implementation.
"""
...
def audio_player_wm8960(mock_pygame_backend, mock_playback_subject):
    ...
    with patch(
        "app.src.module.audio_player.audio_wm8960.PygameAudioBackend",
        return_value=mock_pygame_backend,
    ):
...
def test_audio_wm8960_init(mock_pygame_backend, mock_playback_subject):
    ...
    mock_pygame_backend.register_end_event_callback.assert_called_once_with(
        player._handle_track_end
    )


@pytest.mark.audio
@patch("app.src.module.audio_player.audio_wm8960.MP3")
def test_audio_wm8960_play_track(
    mock_mp3,
    audio_player_wm8960,
    sample_playlist,
    mock_pygame_backend,
    mock_playback_subject,
):
    ...
    expected_loading_track_info = {
        "number": 1,
        "title": "Track 1",
        "filename": "track1.mp3",
        "duration": 180.0,
    }
    mock_playback_subject.notify_playback_status.assert_any_call(
        "loading", expected_loading_playlist_info, expected_loading_track_info
    )
...
@pytest.mark.audio
@patch("app.src.module.audio_player.audio_wm8960.MP3")
def test_audio_wm8960_pause_resume(
    mock_mp3,
    audio_player_wm8960,
    sample_playlist,
    mock_pygame_backend,
...
@pytest.mark.audio
def test_audio_wm8960_stop(
    audio_player_wm8960, sample_playlist, mock_pygame_backend, mock_playback_subject
):
...
def test_audio_wm8960_set_volume(audio_player_wm8960, mock_pygame_backend):
    ...
    mock_pygame_backend.set_volume.assert_called_with(
        75
    )  # Pygame backend expects 0-100
...
def test_audio_wm8960_get_track_duration(mock_mp3, audio_player_wm8960):
    ...
    # or just ensure it's not called for non-mp3
    mock_mp3.side_effect = Exception("Simulating mutagen error for non-mp3")
...
@pytest.mark.audio
@patch("app.src.module.audio_player.audio_wm8960.MP3")
def test_audio_wm8960_handle_track_end(
    mock_mp3, audio_player_wm8960, sample_playlist, mock_playback_subject
):
...
@pytest.mark.audio
def test_audio_wm8960_handle_track_end_at_playlist_end(
    audio_player_wm8960, sample_playlist, mock_playback_subject
):
