...
class TrackProgressService:
    ...
    def __init__(
        self, state_manager: StateManager, audio_controller, interval: Optional[float] = None
    ):
        """Initialize the track progress service.

        Args:
            state_manager: StateManager instance for broadcasting events
            audio_controller: Audio controller for getting playback status
    ...
    async def start(self):
        ...
        if self._running:
            logger.log(LogLevel.WARNING, "TrackProgressService already running")
            return
        ...
        logger.log(
            LogLevel.INFO,
            f"✅ TrackProgressService STARTED - interval: {self.interval}s (should emit every {int(self.interval*1000)}ms)",
        )
    ...
    async def stop(self):
        ...
        if not self._running:
            logger.log(LogLevel.DEBUG, "TrackProgressService already stopped")
            return

        logger.log(LogLevel.INFO, "Stopping TrackProgressService...")
        self._running = False
        ...
        logger.log(LogLevel.INFO, "TrackProgressService stopped successfully")

    @handle_service_errors("track_progress")
    async def _progress_loop(self):
        """Main loop for periodic progress emission with error recovery."""
        logger.log(
            LogLevel.INFO, "🔄 TrackProgressService loop STARTED - will emit position events"
        )
        ...
        while self._running:
            ...
            if self._error_count > 0:
                logger.log(LogLevel.INFO, "✅ Track progress service recovered from errors")
                self._error_count = 0

            # Log every 100 loops (reduce logging frequency)
            if loop_counter % 100 == 0:
                logger.log(
                    LogLevel.DEBUG,
                    f"📍 Progress loop alive - iteration {loop_counter}, errors: {self._error_count}",
                )
    ...
    async def _emit_progress(self):
        ...
        async with self._safe_operation_context("emit_progress"):
            ...
            if not self._running:
                logger.log(
                    LogLevel.WARNING,
                    "❌ TrackProgressService not running - no position updates will be sent",
                )
                return
            ...
            if not self.state_manager:
                logger.log(
                    LogLevel.ERROR,
                    "❌ No StateManager available - cannot broadcast position updates",
                )
                return

            if not self.audio_controller:
                logger.log(LogLevel.DEBUG, "No audio controller available for progress emission")
                return
            ...
            if not hasattr(self, "_first_status_logged"):
                logger.log(LogLevel.INFO, f"🎵 FIRST playback status: {status}")
            ...
            if not status:
                if not hasattr(self, "_no_status_logged"):
                    logger.log(
                        LogLevel.WARNING,
                        f"⚠️️ No status returned from audio controller (attempt #{self._emission_attempt_count})",
                    )
            ...
            if not self._validate_position_data(current_time, duration, track_id):
                if not hasattr(self, "_validation_fail_logged"):
                    logger.log(
                        LogLevel.WARNING,
                        f"❌ VALIDATION FAILED (attempt #{self._emission_attempt_count}): time={current_time}, duration={duration}, track_id={track_id}",
                    )
            ...
            if not hasattr(self, "_first_valid_logged"):
                logger.log(
                    LogLevel.INFO,
                    f"✅ FIRST VALID position: time={current_time:.1f}s, duration={duration:.1f}s, track_id={track_id}, playing={is_playing}",
                )
            ...
            if not hasattr(self, "_first_broadcast_logged"):
                logger.log(
                    LogLevel.INFO,
                    f"📡 FIRST BROADCAST attempt: pos={current_time_ms}ms, track={track_id}, playing={is_playing}",
                )
            ...
            if result is None and not hasattr(self, "_throttle_logged"):
                logger.log(LogLevel.WARNING, "⚠️️ Position update THROTTLED by StateManager")
                self._throttle_logged = True
            elif result and not hasattr(self, "_broadcast_success_logged"):
                logger.log(
                    LogLevel.INFO,
                    f"✅ FIRST BROADCAST SUCCESS: {result.get('event_type', 'unknown')}",
                )
    ...
    def reset_error_count(self):
        ...
        self._error_count = 0
        if old_count > 0:
            logger.log(LogLevel.INFO, f"Error count reset from {old_count} to 0")
    ...
    def configure_error_handling(
        ...
        logger.log(
            LogLevel.INFO,
            f"Error handling configured: max_errors={self._max_consecutive_errors}, "
    ...
    def _reset_diagnostic_attributes(self, preserve_counters: bool = False):
        ...
        if reset_count > 0:
            logger.log(LogLevel.DEBUG, f"Reset {reset_count} diagnostic attributes")
    ...
    async def _check_for_track_change(self, status: dict):
        ...
        if track_changed and track_number is not None:
            logger.log(
                LogLevel.INFO,
                f"🎵 Track change detected: {self._last_track_number} → {track_number}",
            )
            ...
            if hasattr(self.audio_controller, "_audio_service"):
                ...
                logger.log(
                    LogLevel.INFO,
                    f"✅ Broadcasted state:track event for '{track_info.get('title', 'Unknown')}'",
                )
    ...
    async def _check_for_track_end(self, current_time: float, duration: float, is_playing: bool):
        ...
        try:
            ...
            if current_time >= duration - 0.1:
                ...
                logger.log(
                    LogLevel.INFO, f"🔚 Track end detected: {current_time:.1f}s >= {duration:.1f}s"
                )

                # Trigger auto-advance via audio controller
                if hasattr(self.audio_controller, "auto_advance_to_next"):
                    success = await asyncio.get_event_loop().run_in_executor(
                        None, self.audio_controller.auto_advance_to_next
                    )
                    if success:
                        logger.log(LogLevel.INFO, "✅ Auto-advance to next track successful")
                    else:
                        logger.log(LogLevel.INFO, "🔚 End of playlist reached")
                else:
                    logger.log(LogLevel.WARNING, "⚠️️ Audio controller doesn't support auto-advance")

        except Exception as e:
            logger.log(LogLevel.ERROR, f"❌ Error in track end detection: {str(e)}")
