...
class YouTubeDownloader:
    ...
    def _handle_progress(self, progress: dict):
        ...
        if total is None or total == 0:  # total can be None initially
            if (
                ...
                logger.log(
                    LogLevel.DEBUG,
                    f"Progress hook: Not enough data for percentage (total: {total}, downloaded: {downloaded_bytes})",
                )
                return
        else:
    ...
    def _perform_download_blocking(self, url: str, playlist_folder: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.log(
                LogLevel.DEBUG,
                f"Ensured output subfolder exists: {files_output_folder}",
            )
            ...
            logger.log(
                LogLevel.INFO,
                f"Found {len(mp3_files)} MP3 files in {files_output_folder}",
            )
            ...
            if not chapters and mp3_files:
                ...
                if entries:
                    logger.log(
                        LogLevel.DEBUG,
                        f"Playlist '{info.get('title', 'Unknown')}' (ID: {info.get('id', 'Unknown')}): No explicit chapters found. Processing {len(entries)} entries as individual chapters.",
                    )
                    ...
                    for idx, entry in enumerate(entries, 1):
                        ...
                        if matching_file:
                            ...
                            logger.log(
                                LogLevel.WARNING,
                                f"Could not find matching file for playlist entry '{entry_title}'. Using entry title as filename basis.",
                            )
                            processed_files_info.append(
                                {
                                    "title": entry_title,
                ...
                for idx, chapter in enumerate(
                    ...
                    if matching_file:
                        ...
                        logger.log(
                            LogLevel.WARNING,
                            f"Could not find matching file for chapter '{chapter_title}'. Using chapter title as filename basis: {str(Path('files') / f'{chapter_title}.mp3')}",
                        )
                        processed_files_info.append(
                            {
                                "title": chapter_title,
            ...
            if self.progress_callback and self.main_loop:
                ...
                try:
                    ...
                    logger.log(
                        LogLevel.ERROR,
                        f"Error sending final error notification: {cb_e}",
                    )
            raise
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    logger.log(
                        LogLevel.ERROR,
                        f"Error sending async error notification: {cb_e}",
                    )
            raise
