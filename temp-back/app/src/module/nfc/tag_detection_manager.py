...
and logic, separating it from the hardware access concerns.
"""
...
class TagDetectionManager:
    ...
    This class is responsible for determining when a tag event should be
    emitted based on timing and state information, separating this logic
    from hardware access.
    """
    ...
    def process_tag_detection(self, uid_string: str) -> Optional[Dict[str, Any]]:
        ...
        # Tag reappeared after being removed
            ...
            current_time - self._last_no_tag_time >= self.removal_threshold
            and not self._tag_present
        ):
        ...
        # Rate limiting - respect minimum delay between detections
            ...
            current_time - self._last_read_time >= self.cooldown_period
            and not self._tag_present
        ):
    ...
    def process_tag_absence(self):
        ...
        Détecte quand un tag n'est plus présent et émet un événement
        d'absence qui sera traité par le PlaylistController pour mettre
        en pause la lecture.
        """
        current_time = time.time()
        ...
        if self._tag_present:
            ...
            duration_present = 0
            if (
                hasattr(self, "_last_tag_detection_time")
                and self._last_tag_detection_time > 0
            ):
                duration_present = current_time - self._last_tag_detection_time
                logger.log(
                    LogLevel.DEBUG,
                    f"Tag présent pendant {duration_present:.2f} secondes",
                )

            # Journal et notification d'absence
            logger.log(
                LogLevel.INFO,
                f"Tag removed from reader (was present for {duration_present:.2f}s)",
            )
            ...
            # self._last_tag reste inchangé pour permettre la comparaison lors d'une
            # réapparition
